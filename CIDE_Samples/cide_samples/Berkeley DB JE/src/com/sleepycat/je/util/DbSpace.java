 ...
 *
 * $Id: DbSpace.java,v 1.1 2006/05/06 09:00:40 ckaestne Exp $
 */
...
public class DbSpace {
	...
	public DbSpace(Environment env, boolean quiet, boolean details,
			boolean sorted) {
		this(DbInternal.envGetEnvironmentImpl(env), quiet, details, sorted);
	}

	public DbSpace(EnvironmentImpl envImpl, boolean quiet, boolean details,
			boolean sorted) {
	...
	public void print(PrintStream out) throws DatabaseException {
		...
		while (iter.hasNext()) {
			...
			if (details) {
				out.println("File 0x" + Long.toHexString(fileNum.longValue())
						+ ": " + fs);
			}
			totals.add(summary);
			fileIndex += 1;
