...
import coloredide.astview.internal.BindingProperty;
...
public class ASTViewLabelProvider extends LabelProvider implements
	...
	public Color getForeground(Object element) {
		...
		if ((element instanceof ExceptionAttribute)
				&& ((ExceptionAttribute) element).getException() != null)
			return fRed;
		...
		if (element instanceof ASTNode) {
			...
			if (javaElement.getJavaElement() == null
					|| !javaElement.getJavaElement().exists()) {
				return fRed;
			}
			return fDarkGreen;
		}
	...
	public Color getBackground(Object element) {
		...
		if (node != null) {
			...
			if (colors.size() > 0) {
				RGB rgb = FeatureManager.getCombinedRGB(colors);
