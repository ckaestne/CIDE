...
public class CApproxParserTester {
	protected CApproxParserTester tester;

	public ISourceFile runParser(File f) throws FileNotFoundException,
			ParseException {
		CApproxParser parser = new CApproxParser(new OffsetCharStream(
				new FileReader(f)));
		return parser.getRoot();
	}

	public ISourceFile runParser(String fileContent) throws ParseException {
		CApproxParser parser = getParser(fileContent);
		return parser.getRoot();
	}
	...
	protected CApproxParser getParser(String content) {
		return new CApproxParser(new OffsetCharStream(new ByteArrayInputStream(
				content.getBytes())));
	}
}
