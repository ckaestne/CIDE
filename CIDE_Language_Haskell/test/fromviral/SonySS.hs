-- This is a stylesheet optimised for use with the Comp Sc Sony data projector

module SonySS (comment1, comment2, comment3, commentCode
    ,open, open1
  ...
  comment1, comment2, comment3 :: Style;

  comment1        = let fmt = Format SansSerif 24 black True False
                    in Comment green black fmt Nothing;

  comment2        = let fmt = Format SansSerif 18 black False False
                    in Comment yellow gray fmt Nothing;

  comment3        = let fmt = Format SansSerif 20 red False False 
                    in Comment yellow red fmt Nothing;

  commentCode     = let fmt = Format Monospaced 18 black True False
                    in Comment lightYellow black fmt Nothing;

  open name       = let fmt = Format SansSerif 16 red False False 
                    in Comment trans red fmt (Just name);

  open1 name      = let fmt = Format SansSerif 16 red True False
                    in Comment trans red fmt (Just name);
  ...
  -- Create a bare-list style

  bareList, bareList' :: Int -> Style -> Style;
  ...
  table box fmt = 
    let {rowSty = list box fmt;
         y = inflate (boxHeight box)
    } in bareList' y rowSty;

  table' box fmt =
    let {colSty = list' box fmt;
         x = inflate (boxWidth box)
    } in bareList x colSty;


  -- Creat a bare-table style

  bareTable, bareTable' :: Int -> Int -> Style -> Style;

  bareTable x y sty =
    let rowSty = bareList x sty
    in bareList' y rowSty;

  bareTable' x y sty =
    let colSty = bareList' y sty
    in bareList x colSty;
