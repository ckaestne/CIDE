...
stand_alone_deriving :: { LDerivDecl RdrName }
  	: 'deriving' 'instance' inst_type {% checkDerivDecl (sL (comb2 $1 $>) (DerivDecl $3)) }
...
exp10 :: { LHsExpr RdrName }
	...
	| '-' fexp				{ sL (comb2 $1 $>) $ NegApp $2 noSyntaxExpr }
...
scc_annot :: { Located FastString }
	: '_scc_' STRING			{% (addWarning Opt_WarnDeprecations (getLoc $1) (text "_scc_ is deprecated; use an SCC pragma instead")) >>= \_ ->
                                   ( do scc <- getSCC $2; return $ sL (comb2 $1 $>) scc ) }
	| '{-# SCC' STRING '#-}'		{% do scc <- getSCC $2; return $ sL (comb2 $1 $>) scc }
...
--	| STRING			{ sL (getLoc $1) (HsOverLit $! mkHsIsString (getSTRING $1)) }
	...
	| '[d|' cvtopbody '|]'	{% checkDecBrGroup $2 >>= \g -> 
					return (sL (comb2 $1 $>) $ HsBracket (DecBr g)) }
...
dbinds 	:: { Located [LIPBind RdrName] }
	: dbinds ';' dbind		{ let { this = $3; rest = unLoc $1 }
                              in rest `seq` this `seq` sL (comb2 $1 $>) (this : rest) }
	| dbinds ';'			{ sL (comb2 $1 $>) (unLoc $1) }
	| dbind				{ let this = $1 in this `seq` sL (getLoc $1) [this] }
--	| {- empty -}			{ [] }
...
docnext :: { LHsDoc RdrName }
  : DOCNEXT {% case parseHaddockParagraphs (tokenise (getDOCNEXT $1)) of {
      ...
      MyRight doc -> return (sL (getLoc $1) doc) } }
