...
data TypedVal = TVString String
              | TVDouble Double
              | TVFun    (Env (Result TypedVal EvalError) -> Result TypedVal EvalError -> Result TypedVal EvalError)
...
eval :: Env (Result TypedVal EvalError)
     -> Exp TypedVal
    ...
    f env val =
        case eval (addToEnv env x val) exp of
            Result (TVFun g) -> Result $ TVFun h
                where
                h env2 val2 = g (addToEnv env2 x val) val2
            r -> r
